...
@pytest.mark.asyncio
async def test_extract_content_from_url_crawl4ai_docker(fixture_path):
    """Tests content extraction from a URL using Crawl4AI Docker mode.

    This test requires CRAWL4AI_API_URL environment variable to be set
    pointing to a running Crawl4AI Docker instance.
    """
    import os

    # Skip if CRAWL4AI_API_URL is not set
    api_url = os.environ.get("CRAWL4AI_API_URL")
    if not api_url:
        pytest.skip(
            "CRAWL4AI_API_URL environment variable not set - Docker mode test skipped"
        )
    ...
    assert hasattr(result, "source_type")
    assert result.source_type == "url"
    # Check for expected title and content snippets
    assert "Supernova Labs" in result.title
    assert "AI Consulting" in result.title
    # Check that content was extracted and contains relevant keywords
    assert len(result.content) > 100
...
async def test_extract_content_from_xlsx(fixture_path):
    ...
    result = await extract_content(
        dict(file_path=str(xlsx_file), document_engine="simple")
    )

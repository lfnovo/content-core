...
- **`image.py`**: Image analysis using Esperanto vision models. Analyzes images with `AIFactory.create_language()` + `create_image_message()`. Falls back to placeholder if no vision model configured. Exports `extract_image`
- **`video_vision.py`**: Video frame analysis using vision models. Extracts frames with ffmpeg (adaptive sampling), analyzes in parallel with semaphore control. Exports `extract_video_with_vision`, `get_video_duration`, `calculate_frame_params`, `extract_frames`, `cleanup_temp_files`
- **`pdf_vision.py`**: PDF page analysis using vision models. Converts pages to images with pdftoppm (adaptive sampling), analyzes in parallel. Exports `extract_pdf_with_vision`, `get_pdf_page_count`, `calculate_page_params`, `convert_pdf_to_images`
...
- Vision processors (`image.py`, `video_vision.py`, `pdf_vision.py`) import `esperanto` lazily inside functions to avoid import-time failures when esperanto is not installed or has an older version
- Vision processors require `vision_provider` and `vision_model` fields in state — routing in `graph.py` checks these via `_has_vision_config()`
- `video_vision.py` requires ffmpeg/ffprobe; `pdf_vision.py` requires pdftoppm/pdfinfo (poppler-utils)

...
Instantiate against the final candidate before execution. For 2.1.0, compare v2.0.7 with the final candidate: HTML ingestion, engine/configuration
errors, public exceptions, logging, MCP version, packaging and release publication.
Record the concrete SHA and artifact hashes in the run record.
...
| A: release trigger and version identity | Validate workflow syntax; exercise Package version check with matching/mismatched release tags and wheel versions; dry-run Make targets | Only `release: published` triggers Publish; draft/tag preparation does not upload; mismatched versions fail; Package artifacts feed upload | Local workflow checks, no publication | Pre-GO, required for workflow changes |
